...
import { putMeDownTracks, getTrackBySlug } from "@/lib/putMeDownTracks";
import Reveal from "@/app/components/Reveal";
import LinksGrid from "@/app/components/LinksGrid";

const EP_SLUG = "put-me-down";

export function generateStaticParams() {
  return putMeDownTracks.map((track) => ({ track: track.slug }));
}
...
export async function generateMetadata({
  ...
  } by LOBUS, from the "Put Me Down" EP (2022). Stream now.`;
        ...
        <Reveal delay={150}>
          <div className="mt-4 flex items-center gap-3">
            <span className="text-sm text-white/50 [text-shadow:0_1px_8px_rgba(0,0,0,0.8)]">
              From Put Me Down &middot; {ep?.year}
              {track.feature ? ` · feat. ${track.feature}` : ""}
            </span>
          </div>
        </Reveal>
